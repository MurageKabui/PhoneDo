  ...
  <div class="menu">
    ...
    <div class="content">
      <ul>
        ...
        <li v-show="app.store.runningscript" @click="app.stopRunningScript();">
          <span class="danger">
...
if (cs) {
    ...
    <div class="menu">
        ...
        <div class="content">
            <ul>
                ...
                <li @click="app.saveAs(app.store.currentScriptName)" v-show="app.store.currentScriptName"><span><i class="fas fa-file-export"></i> Export Script</span></li>
                <li @click="app.shareScript()" v-show="app.store.currentScriptName"><span><i class="fas fa-share-alt"></i> Share Script Content</span></li>
                <li @click="app.renameScriptAs()" v-show="app.store.currentScriptName"><span><i class="fas fa-i-cursor"></i> Rename Script</span></li>
                ...
                <li @click="app.closeFile()" v-show="app.store.currentScriptName"><span class="warn"><i class="fas fa-times-circle"></i> Close File</span></li>
    ...
    <div class="menu" v-show="app.store.currentScriptName">
        ...
        <div class="content">
            <ul>
                <li @click="app.store.aceedit.undo()"><span><i class="fas fa-undo-alt"></i> Undo</span></li>
                <li @click="app.store.aceedit.redo()"><span><i class="fas fa-redo-alt"></i> Redo</span></li>
                ...
                <li @click="app.store.aceedit.execCommand('selectAll')"><span><i class="fas fa-check-double"></i> Select All</span></li>
                <li @click="app.store.aceedit.execCommand('gotoline')"><span><i class="fas fa-hashtag"></i> Go to Line</span></li>
                <li @click="app.store.aceedit.duplicateLines()"><span><i class="fas fa-copy"></i> Duplicate Selection</span></li>
                ...
                <li @click="app.store.aceedit.execCommand('transposeletters')"><span><i class="fas fa-exchange-alt"></i> Transpose Characters</span></li>
                <li @click="app.store.aceedit.execCommand('togglecomment')"><span><i class="fas fa-comment-dots"></i> Toggle Comment</span></li>
                ...
                <li @click="app.store.aceedit.toUpperCase()"><span><i class="fas fa-uppercase"></i> Convert to Uppercase</span></li>
                <li @click="app.store.aceedit.toLowerCase()"><span><i class="fas fa-lowercase"></i> Convert to Lowercase</span></li>
                <li @click="app.store.aceedit.execCommand('toggleoverwrite')"><span><i class="fas fa-pen-fancy"></i> Toggle Overwrite</span></li>
                <li @click="app.store.aceedit.getSession().foldAll()"><span><i class="fas fa-minus-circle"></i> Fold All Code</span></li>
                <li @click="app.store.aceedit.getSession().unfold()"><span><i class="fas fa-plus-circle"></i> Unfold All Code</span></li>
                <li @click="app.store.aceedit.execCommand('gotostart')"><span><i class="fas fa-chevron-double-up"></i> Jump to Start</span></li>
                <li @click="app.store.aceedit.execCommand('gobottom')"><span><i class="fas fa-chevron-double-down"></i> Jump to End</span></li>
                ...
                <li @click="app.resetEditorSettings()" class="warning-item">
                    <span class="warn"><i class="fas fa-undo"></i> Factory Reset Editor</span>
                </li>
            </ul>
        </div>
    </div>
    ...
    <div class="menu" v-show="app.store.currentScriptName">
        ...
        <div class="content">
            <ul>
                ...
                <li @click="app.tidyPrompt(app.store.currentScriptName)"><span><i class="fas fa-broom"></i> Tidy Script</span></li>
                <li @click="app.minifyScript(app.store.currentScriptName)"><span><i class="fas fa-compress-alt"></i> Minify Script</span></li>
                <li @click="app.pairWorkspace();"><span><i class="fas fa-link"></i> Pair Workspace</span></li>
                <li @click="app.askAI();"><span><i class="fas fa-robot"></i> Ask AI</span></li>
            </ul>
        </div>
    </div>
        ...
        <div class="content">
            <ul>
                <li :class="{active: app.store.currentScriptName == sn}" v-for="(sn, index) in app.store.scriptNames" @click="app.openScript(sn)">
                    <span><i class="fa-solid fa-file-code"></i> {{index + 1}}. {{sn.slice(0, -3)}}</span>
                </li>
        ...
        <div class="content">
            <ul>
                <li :class="{ active: app.store.currentThemeName === theme.name }" v-for="theme in app.store.acetheme" :key="theme.name" @click="app.loadAceTheme(theme.name)">
                    <span><i class="fas fa-palette"></i> {{theme.name.replace(/_/g, ' ')}}</span>
                </li>
            </ul>
        </div>
    </div>
    <div class="menu">
        ...
        <div class="content">
            <ul>
                <li @click="app.browserOpen('https://github.com/MurageKabui/PhoneDo/blob/fa5481ea1ff14e474c60de9f8d33a3bc5225a471/docs/README.MD')"><span><i class="fas fa-book"></i> Documentation</span></li>
                <li @click="app.browserOpen('https://github.com/MurageKabui/PhoneDo')"><span><i class="fas fa-users"></i> Community</span></li>
                <li @click="app.browserOpen('https://github.com/MurageKabui/PhoneDo/tree/main/examples')"><span><i class="fas fa-file-code"></i> Demo Scripts</span></li>
                <li @click="app.showAbout()"><span><i class="fas fa-info-circle"></i> About</span></li>
            </ul>
        </div>
    </div>
...
SA.prompt({
    ...
    body: 'Enter a unique name for your script.',
    textLimit: 25,
    attributes: {
        placeholder: 'eg: Hello.js',
        autocomplete: 'off',
        spellcheck: false
    }
})
...
SA.prompt({
    ...
    // Handle cancel case
    if (newScriptName === null || newScriptName === undefined) {
        return; // User cancelled
    }
    const trimmedName = app.trimStr(newScriptName);
    // Validate: not empty, ends with .js, within length limit
    if (!trimmedName || trimmedName.length > 25 || !trimmedName.endsWith('.js')) {
        return app.errorFileCriteria();
    }

    // Check if name actually changed
    if (trimmedName === scriptName) {
        app.toastMsg('No changes made', 'short');
        return;
    }
    const query = 'UPDATE userScripts SET scriptName = ? WHERE scriptName = ?';
    app.store.phoneDoDB.transaction(function(tx) {
        tx.executeSql(
            query,
            [trimmedName, scriptName],
            function(tx, rs) {
                if (rs.rowsAffected > 0) {
                    app.store.currentScriptName = trimmedName;
                    app.toastMsg(`Renamed "${scriptName}" to "${trimmedName}"`, 'long');
                } else {
                    app.toastMsg(`"${scriptName}" not found!`, 'short');
                }
                app.populateUserScripts();
...
commands.slist = function () {
    ...
    */
    ...
    commands.run = function () {
        ...
        // Return the promise chain and AWAIT evaluate so `run` does not resolve until the
        // script (including any console.prompt() it awaits) has fully finished. Without this,
        // run() returned immediately, the shell re-armed its own prompt, and that shell prompt
        // stole the script's input (you would see: ERROR: command "<answer>" is not recognized).
        return app.getActualScriptContent(script)

            .then(async (scriptContent) => {
...
`,
    ...
    */
    ...
    if (handler) {
        // Return the handler's result so callers can await it. For async commands like `run`
        // this is the promise that settles when the command (and its script) has finished;
        // for synchronous commands it is undefined, which awaits harmlessly.
        return handler();
    }
...
return SA.alert({
    ...
    body: 'Please check your script name:',
    buttons: [
        {
            text: 'OK',
            value: 'ok',
...
function exprt(fileType = '.js'){
   let fileContent = app.store.aceedit.getValue();
   let fileName = ((fileType == '.js') ? sn : sn + '.txt');
...
SA.askAI({
  placeholder: 'Describe the script…',
  onGenerate(prompt, api) {
    // Simulate streaming dummy code
    const dummyLines = [
      '// Dummy script generated for: ' + prompt,
      'function dummyFunction() {',
      '  console.log("This is a dummy script");',
      '  return "Generated at: " + new Date().toLocaleTimeString();',
      '}',
      '',
      '// Example usage',
      'dummyFunction();'
    ...
    // Stream each line with delay
    dummyLines.forEach((line, index) => {
      setTimeout(() => {
        api.append(line + '\n');
        if (index === dummyLines.length - 1) {
          const fullCode = dummyLines.join('\n');
          api.done(fullCode);
          app.store.aceedit.setValue(fullCode);
        }
      }, 200 * (index + 1));
    });

    return Promise.resolve(dummyLines.join('\n'));
  },
  onInsert(code) {
    ...
    stopRunningScript() {
      let
        ...
        frames = this._getLoadedFrames(),
        dialogs = this._getLoadedDialogs();
if (app.store && typeof app.store.stopScript === 'function') {
    ...
    pairWorkspace() {
      let
        ...
        dialogs = this._getLoadedDialogs();
const promptConfig = {
    title: "Pair Workspace",
    body: "Go to www.denniskabui.com/phonedo/workspace and enter your Workspace ID",
    textLimit: 12,
    attributes: {
        placeholder: "eg: ABCD123FFG",
        autocomplete: 'off',
        spellcheck: false,
        autocorrect: 'off',
        autocapitalize: 'characters'  // Useful for workspace IDs
    }
};
...
SA.prompt(promptConfig)
    .then(workspaceid => {
        // Handle cancel case
        if (!workspaceid) {
            app.showAlert('Pairing cancelled');
            return;
        }
        ...
        if (trimmedId.length === 0) {
            app.showAlert('Workspace ID cannot be empty');
            return;
        }

        app.showAlert('Connecting to Workspace ID: ' + trimmedId);
        // Add your connection logic here
    })
    .catch(error => {
        app.showAlert('Error: ' + error.message || 'Failed to get Workspace ID');
    });
    },
...
let handleInput = async function (input) {
    ...
    if (commandName && app.store.commands.hasOwnProperty(commandName)) {
        ...
        // Await the command so the shell pauses until it finishes. Without this, a script
        // launched by `run` keeps executing while the line below re-arms the shell prompt,
        // so the shell prompt steals any input the script asks for via console.prompt()
        // (you would see: ERROR: command "<your answer>" is not recognized).
        await app.processCommand();
    } else {
        if (commandName) {
...
NavigationBar.backgroundColorByHexString("#333333", true);
