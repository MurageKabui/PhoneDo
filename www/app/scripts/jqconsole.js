(function () {
    ...
    Ansi = function () {
        ...
        Ansi.prototype.stylize = function (text) {
            ...
            // Fast path: no ANSI escape in this text AND no style carried over from a previous
            // write -> return it unchanged (skip the state machine and the wrapper span).
            // The klasses check is essential: jqconsole lets style bleed across writes (a color
            // with no RESET colors the next line), so we must NOT skip when a style is active.
            if (text.indexOf(ESCAPE_CHAR) === -1 && this.klasses.length === 0) {
                return text;
            }
            text = this._openSpan(text);
            i = 0;
            // Condition is `!== -1` (not `&& i !== -1`): the old form also exited when i === 0,
            // silently dropping an escape at index 0. Post-_openSpan the text always starts
            // with "<span", so i is never 0 here; this just removes the latent edge case.
            while ((i = text.indexOf(ESCAPE_CHAR, i)) !== -1) {
    ...
    // Plain, allocation-free HTML escape. Produces the same &amp;/&lt;/&gt; output that
    // jQuery's $(span).text(x).html() did, without building throwaway jQuery nodes per write.
    ...
    JQConsole = function () {
        ...
        JQConsole.prototype.Write = function (text, cls, escape) {
            ...
            if (escape) {
                text = this.ansi.stylize(escapeHtml(text))
            }
        ...
        JQConsole.prototype.Clear = function () {
            // Remove the output nodes outright rather than blanking their text. Blanking left
            // the emptied nodes attached, so repeated Clear() calls (e.g. redraw-based
            // animations) leaked a steadily growing pile of dead DOM.
            //
            // Anchor the removal on the actual $prompt element (prevUntil stops at the header),
            // NOT on a class-guess selector: the prompt's class varies with state, so a class
            // match could overrun and delete the prompt/input node itself. This form removes
            // exactly the output siblings between the header and the prompt, and can never
            // touch the header or the live prompt.
            this.$prompt.prevUntil("." + CLASS_HEADER).remove();
        ...
        JQConsole.prototype._InitPrompt = function () {
            ...
            if (!this.isMobile) {
                ...
                // Initialize with a space to ensure backspace events are always fired
        ...
        JQConsole.prototype._ScrollToEnd = function () {
            ...
            // Coalesce the window-scroll: each write used to queue its own setTimeout(..., 50),
            // so a burst of N writes piled up N timers, each doing layout reads. Schedule at
            // most one scroll per animation frame instead.
            if (this.auto_focus && !this._scroll_scheduled) {
                this._scroll_scheduled = true;
                self = this;
                raf = window.requestAnimationFrame || function (cb) { return setTimeout(cb, 16) };
                raf(function () {
                    self._scroll_scheduled = false;
                    self.ScrollWindowToPrompt()
                })
            }
        };
        ...
        JQConsole.prototype._EndComposition = function () {
            ...
            this.$input_source.val(this.isAndroid ? " " : "");
        ...
        JQConsole.prototype._UpdateComposition = function (e) {
            ...
            cb = function (_this) {
                return function () {
                    var val;
                    if (!_this.in_composition && !_this.isAndroid) {
                        return
                    }
                    val = _this.$input_source.val();
                    if (_this.isAndroid) {
                        if (val.length === 0) {
                            // Backspace was pressed (deleted the "ghost space")
                            _this._Backspace();
                            _this.$input_source.val(" ");
                        } else if (val.length > 1 && val[0] === " ") {
                            // User typed something after the ghost space
                            var text = val.slice(1);
                            _this.$prompt_left.text(_this.$prompt_left.text() + text);
                            _this.$input_source.val(" ");
                            _this._ScrollToEnd();
                        } else if (val.length > 0 && val[0] !== " ") {
                            // The ghost space was replaced by a character (some keyboards do this)
                            _this.$prompt_left.text(_this.$prompt_left.text() + val);
                            _this.$input_source.val(" ");
                            _this._ScrollToEnd();
                        }
                    } else {
                        return _this.$composition.text(val)
                    }
                }
            }(this);
